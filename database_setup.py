import os
import psycopg2

# Connect to the database
conn = psycopg2.connect(os.environ['DATABASE_URL'])

# Create a cursor object
cur = conn.cursor()

# Create the dentists table if it doesn't exist
cur.execute('''
CREATE TABLE IF NOT EXISTS dentists (
    id SERIAL PRIMARY KEY,
    name TEXT NOT NULL,
    email TEXT NOT NULL UNIQUE,
    password TEXT NOT NULL
)
''')

# Create the patients table if it doesn't exist
cur.execute('''
CREATE TABLE IF NOT EXISTS patients (
    id SERIAL PRIMARY KEY,
    name TEXT NOT NULL,
    date_of_birth TEXT NOT NULL,
    age INTEGER,
    gender TEXT,
    height REAL,
    weight REAL,
    diabetes_mellitus_type_ii BOOLEAN,
    diabetes_mellitus_type_ii_date TEXT,
    hta BOOLEAN,
    hta_date TEXT,
    lung_diseases BOOLEAN,
    lung_diseases_date TEXT,
    gastrointestinal_diseases BOOLEAN,
    gastrointestinal_diseases_date TEXT,
    liver_diseases BOOLEAN,
    liver_diseases_date TEXT,
    neurological_diseases BOOLEAN,
    neurological_diseases_date TEXT,
    cancer BOOLEAN,
    cancer_date TEXT,
    osteoarticular_diseases BOOLEAN,
    osteoarticular_diseases_date TEXT,
    sexual_diseases BOOLEAN,
    sexual_diseases_date TEXT,
    dentist_id INTEGER,
    FOREIGN KEY (dentist_id) REFERENCES dentists (id)
)
''')

# Create the dental_charts table if it doesn't exist
cur.execute('''
CREATE TABLE IF NOT EXISTS dental_charts (
    id SERIAL PRIMARY KEY,
    patient_id INTEGER,
    chart_data TEXT NOT NULL,
    FOREIGN KEY (patient_id) REFERENCES patients (id)
)
''')

# Commit the changes and close the connection
conn.commit()
cur.close()
conn.close()

print("Database and tables created successfully.")
